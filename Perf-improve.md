...
#> 1 mean1(x)    187.6μs  200.8μs     4572.
#> 2 mean2(x)     92.1μs   97.4μs     9924.
```
...
#>   expression           min   median `itr/sec`
#>   <bch:expr>      <bch:tm> <bch:tm>     <dbl>
#> 1 mean(x)            3.9μs    4.8μs   139799.
#> 2 mean.default(x)    1.2μs      2μs   346362.
```
...
#>   expression              min   median `itr/sec`
#>   <bch:expr>         <bch:tm> <bch:tm>     <dbl>
#> 1 mean(x)               3.9μs    4.9μs   150959.
#> 2 mean.default(x)       1.2μs    1.5μs   477724.
#> 3 .Internal(mean(x))    200ns    300ns  2847137.
```
...
#>   expression              min   median `itr/sec`
#>   <bch:expr>         <bch:tm> <bch:tm>     <dbl>
#> 1 mean(x)                22μs   23.4μs    34688.
#> 2 mean.default(x)      18.8μs   19.1μs    49807.
#> 3 .Internal(mean(x))   17.8μs   17.9μs    54001.
```
...
#>   expression         min   median `itr/sec`
#>   <bch:expr>    <bch:tm> <bch:tm>     <dbl>
#> 1 as.data.frame   1.29ms    1.5ms      564.
#> 2 quick_df         7.4μs    8.8μs    99644.
```
...
#>   expression        min   median `itr/sec`
#>   <bch:expr>   <bch:tm> <bch:tm>     <dbl>
#> 1 lookup[x1]      500ns    700ns  1093422.
#> 2 lookup[x10]     1.3μs    1.7μs   404783.
#> 3 lookup[x100]    3.1μs      6μs   145745.
```
...
#>   expression      min   median `itr/sec`
#>   <bch:expr> <bch:tm> <bch:tm>     <dbl>
#> 1 loop10         20μs     23μs    37988.
#> 2 loop100     740.5μs  786.5μs     1241.
#> 3 vec10         4.6μs      5μs   182604.
#> 4 vec100       34.5μs   36.1μs    25878.
```
