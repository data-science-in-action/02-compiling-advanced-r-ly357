...
1.  Predict what each of the following quosures will return if
    ...
    #> <quosure>
    #> expr: ^x
    #> env:  0000000019AFB050
    
    q2 <- new_quosure(expr(x + !!q1), env(x = 10))
    q2
    #> <quosure>
    #> expr: ^x + (^x)
    #> env:  0000000019C313C0
    
    q3 <- new_quosure(expr(x + !!q2), env(x = 100))
    q3
    #> <quosure>
    #> expr: ^x + (^x + (^x))
    #> env:  0000000019E8AB90
    ```
...
subsample(df, x == 1)
#> Error in eval_tidy(rows, data): 找不到对象'x'
```
...
*   `base::subset()` always evaluates `rows` in the calling environment, but
    ...
    #> Error in eval(rows, data, caller_env()): 找不到对象'zzz'
    ```
...
#> Error in is.data.frame(data): 找不到对象'resample_data'
```
